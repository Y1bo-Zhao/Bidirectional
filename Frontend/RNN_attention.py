# -*- coding: utf-8 -*-
"""
train_rnn_attention_holdout.py

- 从 dataset/ 读取 sample_*.npy / label_*.npy
- 标签支持 {0,1} 或 [0,1] 连续值（软标签）
- 模型：GRU + Attention → 1 个 logit（经 sigmoid 得到风险概率）
- 训练：BCEWithLogitsLoss（兼容软标签）
- 验证：val_MSE / val_MAE / Pearson corr（若是0/1标签会额外打印 acc@0.5，仅参考）
- 划分：分层留出（连续标签按分位数分桶）
- 可视化：单张大图（2×2 子图）同时展示：Train Loss / Val MSE&MAE / Val Corr / 验证散点图
"""
...
class RNNWithAttention(nn.Module):
    def __init__(self, input_dim: int, hidden_size=64, num_layers=1):
        super().__init__()
        self.gru  = nn.GRU(input_dim, hidden_size, num_layers=num_layers, batch_first=True)
        self.attn = AttnHead(hidden_size)
...
def train_holdout(ds,
    ...
    # 模型/损失/优化
    model = RNNWithAttention(ds.D, hidden_size, NUM_LAYERS).to(device)
    ...
    for ep in range(1, epochs+1):
        ...
        final_metrics = metrics  # 记录最后一轮的指标与 preds/gts 供可视化
    ...
    if best_state is not None:
        torch.save(best_state, os.path.join(save_dir, "rnn_attention_best.pth"))
...
def main():
    ...
    print(line)
    print(f"最佳模型已保存至 {os.path.join(SAVE_DIR, 'rnn_attention_best.pth')}")

    # ====== 单张大图（2×2 子图）集中展示所有图表 ======
